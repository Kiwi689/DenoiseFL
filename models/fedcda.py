...
class FedCDA(FederatedModel):
    ...
    def __init__(self, nets_list, args, transform):
        ...
        # These are the "selected and fixed local models" kept by server
    ...
    def ini(self):
        ...
        # Initialize fixed models with initial global model
    ...
    def loc_update(self, priloader_list):
        ...
        # Warmup: standard FedAvg-style aggregation on current online clients
    ...
    def _train_net(self, index, net, train_loader):
        ...
        for local_ep in range(self.local_epoch):
            ...
            # Keep only the last local epoch loss, following paper description
    ...
    def _objective_value_for_selection(self, assembled_client_ids, assembled_model_loss_pairs):
        ...
        This is close to the paper's Eq.(6)/(8).
        """
    ...
    def _build_context_for_current_batch(self, processed_online_clients, current_batch_choices):
        ...
        for cid in all_clients:
            if cid in current_batch_set:
                ...
                # Future online clients are excluded in current batch approximation
                continue

        return assembled_client_ids, assembled_pairs

    def _get_candidates_for_client(self, cid):
        ...
        candidates = []
        for state_i, loss_i in zip(history_states, history_losses):
            candidates.append((copy.deepcopy(state_i), float(loss_i)))
        return candidates

    def _select_models_for_batch_joint(self, batch_client_ids, processed_online_clients):
        """
        Jointly select one cached model for each client in the current batch
        by enumerating all candidate combinations inside the batch.

        This is closer to the paper than the previous per-client greedy selection.
        """
        if len(batch_client_ids) == 0:
            return {}

        client_candidates = {}
        for cid in batch_client_ids:
            client_candidates[cid] = self._get_candidates_for_client(cid)

        candidate_lists = [client_candidates[cid] for cid in batch_client_ids]

        best_obj = None
        best_selection = None

        for combo in product(*candidate_lists):
            current_batch_choices = {
                cid: pair for cid, pair in zip(batch_client_ids, combo)
            }
            ...
            if (best_obj is None) or (obj < best_obj):
                best_obj = obj
                best_selection = {
                    cid: (copy.deepcopy(pair[0]), float(pair[1]))
                    for cid, pair in current_batch_choices.items()
                }
    ...
    def aggregate_nets(self, freq=None):
        ...
        2) Jointly select one cached model for each client in a batch
        ...
        for batch_client_ids in batches:
            selected_for_batch = self._select_models_for_batch_joint(
